...
import { normalizeVideoConfig, normalizeVideoResolutionValue, videoDurationRule, videoParameterOptions, videoReferenceMode, isAgnesVideoV25Model, supportsVideoAudioGeneration, supportsVideoFrameReferences } from "@/lib/video-model-capabilities";
...
export default function VideoPage() {
    ...
    const referenceImageLimit = SEEDANCE_REFERENCE_LIMITS.images;
    const videoReferenceLimit = SEEDANCE_REFERENCE_LIMITS.videos;
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        const videoFiles = selectedFiles.filter((file) => file.type.startsWith("video/") && file.size <= SEEDANCE_REFERENCE_LIMITS.videoMaxBytes).slice(0, Math.max(0, videoReferenceLimit - videoReferences.length));
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        ...
        const insertAudio = () => {
            ...
            const next = filterAudioReferencesByDuration(audioReferences, [{ id: nanoid(), name: payload.title, type: payload.mimeType || "audio/mpeg", url: payload.url, storageKey: payload.storageKey, bytes: payload.bytes, durationMs: payload.durationMs }], message.warning);
